...
const defaultAboutSettings = {
  ...
  stats: {
    yearsExperience: "5+", // Updated as requested by user
...
async function seed() {
  ...
  try {
    ...
    for (const [key, data] of entries) {
      await client.query(
        `INSERT INTO public.clinic_settings (key, data, updated_at) 
         VALUES ($1, $2, NOW())
         ON CONFLICT (key) DO UPDATE SET data = EXCLUDED.data, updated_at = NOW();`,
        [key, JSON.stringify(data)]
      );
      console.log(`Seeded ${key} in clinic_settings.`);
    }

    const res = await client.query('SELECT key, updated_at FROM public.clinic_settings;');
    console.log('Current rows in clinic_settings:', res.rows);
