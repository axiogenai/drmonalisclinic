    ...
    <div className="fixed inset-0 z-[130] overflow-hidden">
      ...
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-3 sm:pl-10">
        <div className="w-screen max-w-md bg-white shadow-2xl flex flex-col justify-between animate-in slide-in-from-right duration-300">
          ...
          {/* Header */}
          <div className="p-3.5 sm:p-6 border-b border-gray-100 flex items-center justify-between bg-white">
            <div className="flex items-center gap-2 sm:gap-2.5">
              {checkoutStep === 'delivery' ? (
                ...
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-[#108283]/10 flex items-center justify-center text-[#108283] shrink-0">
                  <ShoppingBag className="w-4 h-4 sm:w-5 sm:h-5" />
                </div>
              )}
              <div>
                <h3 className="font-['Playfair_Display'] text-base sm:text-xl font-bold text-gray-900 leading-tight">
            ...
            <button 
              onClick={closeCart}
              className="p-1.5 sm:p-2 rounded-full hover:bg-gray-100 text-gray-500 hover:text-gray-900 transition-colors cursor-pointer"
            >
          ...
          {checkoutStep === 'cart' ? (
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-2 sm:space-y-3.5">
              {items.length === 0 ? (
                ...
                items.map(({ product, quantity }) => (
                  <div 
                    key={product.id}
                    className="flex items-center gap-2.5 sm:gap-3.5 p-2 sm:p-3.5 bg-gray-50/80 hover:bg-gray-50 rounded-xl sm:rounded-2xl border border-gray-100/90 relative group transition-all"
                  >
                    <div className="w-13 h-13 sm:w-16 sm:h-16 rounded-lg sm:rounded-xl overflow-hidden bg-white shrink-0 border border-gray-200/80 p-0.5 shadow-2xs">
                      <img 
                        src={product.image} 
                        alt={product.name}
                    ...
                    <div className="flex-1 flex flex-col justify-between min-w-0 pr-0.5 sm:pr-1">
                      <div className="flex justify-between items-start gap-1.5 sm:gap-2">
                        <div className="min-w-0">
                          <h4 className="font-['Playfair_Display'] text-xs sm:text-sm font-bold text-gray-900 truncate leading-snug" title={product.name}>
                            {product.name}
                          </h4>
                        ...
                        <button 
                          ...
                          className="text-gray-400 hover:text-rose-600 transition-colors p-1 -mr-1 -mt-0.5 cursor-pointer shrink-0"
                          title="Remove item"
                        >
                      ...
                      <div className="flex justify-between items-center mt-1 sm:mt-2">
                        <span className="font-bold text-xs sm:text-sm text-gray-950 font-['Source_Sans_3']">
                          ₹{product.price * quantity}
                        </span>

                        <div className="flex items-center gap-0.5 sm:gap-1 bg-white border border-gray-200 rounded-md sm:rounded-lg p-0.5 shadow-2xs">
                          <button 
                            onClick={() => updateQuantity(product.id, quantity - 1)}
                            className="p-1 sm:p-1 text-gray-500 hover:text-gray-900 transition-colors cursor-pointer"
                          >
                            <Minus className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                          </button>
                          <span className="px-1.5 sm:px-2 text-[11px] sm:text-xs font-bold text-gray-800 font-mono">
                            {quantity}
                          </span>
                          <button 
                            onClick={() => updateQuantity(product.id, quantity + 1)}
          ...
          {/* Footer Summary */}
          {items.length > 0 && (
            <div className="p-3.5 sm:p-6 border-t border-gray-100 bg-[#FAF0DD]/30 space-y-2.5 sm:space-y-3 font-['Source_Sans_3']">
